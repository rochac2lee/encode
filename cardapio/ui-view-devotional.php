...
<div class="row">
	...
	$select = "SELECT * from devocionais WHERE id = '$id'";
	...
	if ($data = $result -> fetch()) {
		do {
			...
			$idDevocional     = $data['id'];
			$tituloDevocional = $data['titulo'];
			$bannerDevocional = $data['banner'];
			$textoBiblicoDevocional = $data['textoBiblico'];
			$textoDevocional = $data['texto'];
			$autorDevocional  = $data['autor'];
			$dataDevocional   = date("d/m/Y", strtotime($data['data_hora_cadastro']));
			$horaDevocional   = date("H:i", strtotime($data['data_hora_cadastro']));
		...
		<div class="card mb-3" style="background: transparent; box-shadow: none;">
			...
			<div class="card-header top_dark">
				...
				<div style="width: 70%; margin-left: 12%;position: absolute; z-index: 0">
					<strong><h5 class="titleBar">Devocional</h5></strong>
				</div>
			</div>
					...
					<div class="tab-content background_dark" id="nav-tabContent">
					  <div class="tab-pane fade show active" style="padding: 0px" id="nav-about" role="tabpanel" aria-labelledby="nav-about-tab">
							<?php
								...
								echo "
								<img style='width: 100%; margin-bottom: 10px; object-fit: cover; height: 170px' src='assets/uploads/devocional/$bannerDevocional'>

								<div style='padding: 5px'>
									<center>
										<div class='clearfix'></div>
										<h4>$tituloDevocional</h4>
								...
								<!-- AddToAny BEGIN -->
								<div class="a2a_kit a2a_kit_size_32 a2a_default_style" style="text-align: center; display: flex; justify-content: center;">
									<center>
